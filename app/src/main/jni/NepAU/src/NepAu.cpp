...
uintptr_t Address::offset() const { return g_addr - g_base; }
uintptr_t Address::addr() const { return g_addr; }
...
Address NativeLibrary::get_pattern(const std::string& pattern) {
    ...
    if (!module.name.empty()) {
        ...
        auto* start = reinterpret_cast<uint8_t*>(module.start_address);
        ...
        for (uint8_t* current_byte = start; current_byte < (start + module.size); ++current_byte) {
            if (*pat == '?' || *current_byte == strtoul(pat, nullptr, 16)) {
                ...
                if (!pat[2]) {
                    return {module.start_address, reinterpret_cast<uintptr_t>(first_match)};
                }
